...
class InitialChunker:
    """Create initial chunks of 256 tokens each"""
    
    def __init__(self, tokenizer_name: str = "cl100k_base", chunk_size: int = 256):
    ...
    def _filter_valid_chunks(self, chunks: List[str]) -> List[str]:
        ...
        for chunk in chunks:
            chunk = chunk.strip()
            if chunk and len(self.tokenizer.encode(chunk)) >= 10:  # Minimum 10 tokens
                valid_chunks.append(chunk)
        
        return valid_chunks
    
    def save_chunks(self, chunks: List[str], output_file: str = "initial_chunks.txt"):
        """Save chunks to file"""
